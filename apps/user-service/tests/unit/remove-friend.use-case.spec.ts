...
describe('RemoveFriendUseCase (Unit)', () => {
  ...
  it('should delete friendship and emit FRIEND_REMOVED event', async () => {
    friendshipRepoMock.findByUsers.resolves({ id: 'f1', userId1: 'user1', userId2: 'user2' });

    await useCase.execute('user1', 'user2');

    expect(friendshipRepoMock.deleteByUsers.calledOnceWith('user1', 'user2')).to.equal(true);
    expect(kafkaProducerMock.emit.calledOnce).to.equal(true);
    const emitArgs = kafkaProducerMock.emit.firstCall.args as [string, { userId: string; friendId: string }];
